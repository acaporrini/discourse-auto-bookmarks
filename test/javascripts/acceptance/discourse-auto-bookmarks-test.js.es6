...
acceptance("discourse-auto-bookmarks", function (needs) {
  needs.user();
  ...
  test("add ID to header", (assert) => {
    ...
    fillIn(
      ".d-editor-input",
      "# This is my H1 header for the test\n## This is h2, with special characters!"
    );

    andThen(() =>
      assert.equal(
        find("#this-is-my-h1-header-for-the-test").text(),
        "This is my H1 header for the test"
      )
    );
    andThen(() =>
      assert.equal(
        find("#this-is-h2-with-special-characters").text(),
        "This is h2, with special characters!"
      )
    );
  });
  ...
  test("show bookmark link on hover", (assert) => {
    ...
    andThen(function () {
      a = find("a[href='#this-is-my-h1-header-for-the-test']");
